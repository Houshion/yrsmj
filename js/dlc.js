...
function dlcUrl() {
    return 'http://yrshoumaiji.app.xiaozhuschool.com';
}
...
//alert(replaceURL('http://*.com/a=1/catid=2/b=3', 'catid', '99'));
//alert(replaceURL('http://*.com/a/1/catid/2/b/3', 'catid', '99'));
function replaceURL(url, name, val, fmt){
...
function redirect() {
    if(isWX_Allipay() == 'WX'){
        location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx42a447a217e4de78&redirect_uri=' + replaceURL(replaceURL(location.href, 'code'), 'state').replace(/&/g, '%26') + '&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect';
    }else{
...
var isauthing = localStorage.getItem('isauthing');
setTimeout(function(){localStorage.removeItem('isauthing');}, 12000);

function dlc_request(method, data, cb, type) {
	if(isauthing && method.indexOf('/api/public/weixin_login') == -1){
		var url = replaceURL(replaceURL(location.href, 'code'), 'state');
		isLogin(function(res1){if(res1.code==1)location.href=url;});
		return;
	}
    var data = data || {};
    data.openid = user_token();
    $.ajax({
        type: type?type:'post',
        url: dlcUrl() + method,
        data: data,
        ...
        success: function(res) {
            if(res.code=='101'){
                clear_token();
                dlctipbox.show('登录已过期,请重重新登录');
				if(isauthing)return;
				isauthing = true;
				localStorage.setItem('isauthing', 1);
				location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx42a447a217e4de78&redirect_uri=' + replaceURL(replaceURL(location.href, 'code'), 'state').replace(/&/g, '%26') + '&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect';		
                return false;
            }
			localStorage.removeItem('isauthing');
            if (cb) cb(res);
        },
        error: function(err) {
            if (err) cb(err)
    ...
    if(!token){//获取不到token
        if(isWX_Allipay()=='WX' ){//如果是微信客户端
            if(getUrlParam('code')==''){
                ...
                dlc_request('/api/public/weixin_login?code='+getUrlParam('code'),{},function(res){
					console.log(res);
                    if(res.code == 1){
						token = res.data.openid;
                        user_save(res.data.openid);
...
function showLoading(msg){
    if(document.getElementsByClassName('masks').length == 0){
        var str='<div class="masks"><div style="position: fixed;top: 0;left: 0;width: 100%;height: 100%;background:#fff;z-index: 99;"></div>;<div style="position: absolute;top: 3rem;left: 0;z-index: 100;width:100%;text-align: center;"><img src="../img/loading.gif" style="width: 4rem;"/><p style="font-size:.32rem;" id="masks_msg">'+(msg?msg:'')+'</p></div></div>'
        $('body').append(str);
    }else{
...
var dlctipbox = {
    ...
    confirm_img: function(msg, cb,msg2) {
        ...
        var html = '<div id="confirm_img" class="bw border-r1 pos_f tac dpn" style="z-index:9527;top: 3.4rem;width: 6.5rem;left: 50%;margin-left: -3.25rem;overflow: hidden;">' +
            '<div class="mt60">' +
            '<img src="../img/icon7.png" style="width: 1.3rem;">' +
            '</div>' +
           '<div class="ptb30">' +
            '<p class="font17 plr40">' + msg + '</p>' +
            (msg2?'<p class="plr40 mt10 col9">' +msg2+ '</p>':'')+
            '</div>'+
            '<div class="font16 lh100 flex_a tac bdt">' +
            '<a class="flex1 confirm_btn col9" onclick="dlctipbox.confirm_img_callback(0)">查看积分</a>' +
            '<a class="colw bgc1 flex1 confirm_btn" onclick="dlctipbox.confirm_img_callback(1)">返回订单</a>' +
            '</div>' +
            '</div>';
    ...
    alert_img: function(msg, img) {
        ...
        var html = '<div id="alert_img" class="bw font16 border-r1 pos_f tac dpn" style="z-index:9527;top: 3.4rem;width: 6.5rem;left: 50%;margin-left: -3.25rem;overflow: hidden;">' +
            '<div class="mt60">' +
            '<img src="' + (img ? img : '../img/r_timeout.png') + '" style="width: 1.4rem;">' +
            '</div>' +
            '<p class="lh40 plr60 mb60 mt30">' + msg + '</p>' +
